...
if($_REQUEST['action'] == "brkcodesub"){
    ...
    $brkstartcode = isset($_POST['brkstartcode'])?trim($_POST['brkstartcode']):"";

    $check = dbQuery($dbConn, "SELECT id,brkstartcode from staff_job_breaks where job_id = '".$jobid."' and staff_id = '".$userid."' and breakday = '".$today."' order by id desc limit 0,1");
    if(dbNumRows($check) > 0){
        $getcode = dbFetchArray($check);
        if($getcode['brkstartcode'] == $brkstartcode){
            dbQuery($dbConn, "UPDATE staff_job_breaks SET brkstarttime = '".$currtime."' where job_id = '".$jobid."' and staff_id = '".$userid."' and brkstartcode = '".$brkstartcode."' and breakday = '".$today."'");
            ...
            $smsbody = "Staff has taken break for the job: ".stripslashes($fetch['title']).".";
            ...
            if($rowuser['notified'] == "Email" || $rowuser['notified'] == "Both"){
				...
				$mail->Subject = "Break has been started";
				$mail->isHTML(true);
				$mail->AddAddress($rowuser['email']);
		
            //$to = $rowuser['email'];
            //$subject = "Break has been started";
            ...
            <tr>
            <td colspan='2' style='padding-left:10px;'>Break has been started for the job: ".stripslashes($fetch['title']).".</td>
            </tr>
            <tr><td colspan='2'>&nbsp;</td></tr>
            <tr>
		  ...
		  if(!$mail->Send()) {
            ...
            $smsbody = "Break has been started for the job: ".stripslashes($fetch['title']).".";
