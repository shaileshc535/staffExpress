...
if($_REQUEST['action'] == "send_note"){
    $dbConn = establishcon();

    $jobid = isset($_POST['jobid'])?trim($_POST['jobid']):"";
    $userid = isset($_POST['userid'])?trim($_POST['userid']):"";
	$empnote = isset($_POST['empnote'])?strip_tags(tep_db_input($_POST['empnote'])):"";
	
	
	dbQuery($dbConn, "UPDATE job_status set employer_note = '".$empnote."' where jobid = '".$jobid."' and application_sent_to = '".$userid."'");
	...
	if($row['notified'] == "Email" || $row['notified'] == "Both"){
		...
		$mail->Subject = "Joining Confirmation Note Sent by Employer";
		$mail->isHTML(true);
		$mail->AddAddress($row['email']);

    //$to = $row['email'];
    //$subject = "Joining Confirmation Note Sent by Employer";
    ...
    <tr>
    <td colspan='2' style='padding-left:10px;'>The employer ".stripslashes($fetch['name'])." has sent you a joining confirmation note regarding the job: ".stripslashes($fetch['title']).".</td>
    </tr>
    <tr><td colspan='2'>&nbsp;</td></tr>
	<tr>
    <td colspan='2' style='padding-left:10px;'>The note is: ".$empnote."</td>
    </tr>
    <tr><td colspan='2'>&nbsp;</td></tr>
    <tr>
    ...
    $smsbody = "The employer ".stripslashes($fetch['name'])." has sent you a joining confirmation note regarding the job: ".stripslashes($fetch['title']).".<br>The note is: ".$empnote.".";
