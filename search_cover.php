...
if(isset($_GET['myaddress']) && $_GET['myaddress']!=""){
	
	$inner_join = "";

    $myaddress = isset($_GET['myaddress'])?tep_db_input($_GET['myaddress']):"";

	$mydist = isset($_GET['mydist'])?tep_db_input($_GET['mydist']):"";
    ...
    if($mylatlon != ""){
		...
		$mytotal_cover = $jobs = dbQuery($dbConn, "Select FirstCut.*,

                acos(sin(".deg2rad($lat).")*sin(radians(lat)) + cos(".deg2rad($lat).")*cos(radians(lat))*cos(radians(lon)-".deg2rad($lon).")) * $R As Dist

            From (

               SELECT a.id,a.title,a.lat,a.lon,a.street_address,a.location,a.postdate,a.jobdate,a.jobdate2,a.starttime,a.endtime,a.workmode,a.add_time,a.covertype,a.longstartdt,b.company_img, (acos(sin(".deg2rad($lat).")*sin(radians(a.lat)) + cos(".deg2rad($lat).")*cos(radians(a.lat))*cos(radians(a.lon)-".deg2rad($lon).")) * $R) as actualdist from job_details a ".$inner_join." inner join users b on a.employer_id=b.id where 1  ".$cond." and a.title != '' and a.location != '' and a.lat != '' and a.workmode != '' and a.isclosed=0 and a.postcomplete=1 group by a.id order by actualdist asc
        ...
        $sql = "Select FirstCut.*,
            ...
            ) As FirstCut limit ".$start.",".$records_per_page."";
...
$zoom = 5;
    ...
    if(isset($_REQUEST['error']) || count($_GET) == 0 || $hasrecord == 0){
		
		$total_cover = total_open_jobs($dbConn);
		

        $myjob = dbQuery($dbConn, "SELECT a.id,a.title,a.lat,a.lon FROM job_details a where a.title != '' and a.location != '' and a.lat != '' and a.workmode != '' and a.isclosed=0 and a.postcomplete=1 order by a.id desc limit ".$start.",".$records_per_page."");
					...
					<div class="col-sm-12" style="text-align:center;">
						...
						<div class="advnserch">
							...
							<div id="filter">
								...
								<form action="" method="get" id="jobpost">
									...
									<input class="form-control" id="showcatssrch" name="showcats" type="text" placeholder="Job Category" readonly />
									<div id="allcatsrch" style="display:none;">
									<ul class="items">
								...
								<?php
									...
									while($catrow = dbFetchArray($cat)){
										?>
										<li>
										<input id="maincat_<?php echo $catrow['id'];?>" type="checkbox" name="maincatid[]" class="emplyrcatsrch" value="<?php echo $catrow['id'];?>" />&nbsp; <?php echo stripslashes($catrow['category']);?>
								...
								?>
									...
									<input class="datepicker form-control" id="from" name="from" type="text" placeholder="Start Date" value="<?php echo $from;?>" />
									...
									<input class="datepicker form-control" id="to" name="to" type="text" placeholder="End Date" value="<?php echo $to;?>" />
                                    ...
                                    <input type="text" class="form-control required" id="myaddress" placeholder="Location (suburb)" name="myaddress" value="<?php echo $myaddress;?>" autocomplete="off" />
					...
					<div class="col-lg-7 col-md-12 col-sm-7">
									...
									<div class="sercoversingle">
										...
										<div class="covrsinglright">
                                            ...
                                            <ul class="homebtn" id="applybtn_<?php echo $row['id'];?>" style="">
                                                ...
                                                <li><a href="<?php echo SITEURL;?>viewmyjob/<?php echo base64_encode($row['id']);?>?myaddress=<?php echo $myaddress;?>&mycatid=<?php echo $catid;?>&workmode=<?php echo $workmode;?>&from=<?php echo $from;?>&to=<?php echo $to;?>" style="padding:0.4rem 0rem;">View</a></li>
						...
						for($i = 1; $i<=ceil($total_cover/$records_per_page); $i++){
							...
							<?php
								...
								?>
									<li><a href="<?php echo SITEURL;?>searchcover/?page=<?php echo $i;?>&myaddress=<?php echo $myaddress;?>&mycatid=<?php echo $catid;?>&workmode=<?php echo $workmode;?>&from=<?php echo $from;?>&to=<?php echo $to;?>"><?php echo $i;?></a></li>
								<?php
							}
						}
						?>
