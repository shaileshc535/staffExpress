...
if($_REQUEST['do'] == "viewjob"){

    $dbConn = establishcon();

    $id = isset($_REQUEST['job'])?base64_decode($_REQUEST['job']):"";

    $user = isset($_REQUEST['user'])?base64_decode($_REQUEST['user']):"";

    $sent = array();

    $check = dbQuery($dbConn, "SELECT application_sent_to from job_status where jobid = '".$id."'");
...
/*if(!isset($_SESSION['loginUserId'])){
		...
		echo "<script>location.href='".SITEURL."staff_login?job=".$id."&user=".$user."'</script>";
...
<div class="works works_inner viethejobbk">
						...
						<div class="viewthejobmaindescribe">
							...
							<?php
									...
									//if(isset($_SESSION['loginUserId'])){
										...
										$usertype = getUserType($dbConn, $user);

										if($usertype == 2){

											$getuser = dbQuery($dbConn, "SELECT isdeactivated from users where id = '".$user."'");
									...
									?>
										...
										$contact = dbQuery($dbConn, "SELECT contacting,hired,avlblty from job_status where jobid = '".$id."' and application_sent_to = '".$user."'");
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
	  ...
	  <form action="" method="post" id="avl_form_direct">
	  ...
	  <div id="applydiv">
		...
		<div class="row">
		<div class="col-md-4">
		<a href="javascript:void(0);" id="accept_offer_direct" class="jobapplybutn" style="display:none;">I am interested in this cover</a>
		</div>
		</div>
		<br>
		</div>
