...
if($_REQUEST['action'] == "approvemore"){
    $dbConn = establishcon();
    ...
    $row = dbFetchArray($check);
        if($row['extra_hours'] != 0){
            ...
            dbQuery($dbConn, "UPDATE staff_job_payment set extra_hours_approved=1 where job_id = '".$jobid."' and staff_id = '".$userid."'");
            ...
            $to = $rowuser['email'];
            $subject = "Employer Has Approved Your Request for Extra Hours";
            ...
            <tr>
            <td colspan='2' style='padding-left:10px;'>Your employer has approved request for ".$row['extra_hours']." extra hours for the job: ".stripslashes($fetch['title']).".</td>
            </tr>
            <tr><td colspan='2'>&nbsp;</td></tr>
            <tr>
            <td colspan='2' style='padding-left:10px;'>New end time is: ".date('M j, Y', strtotime($newenddate))." at ".date('h:i a', strtotime($newendtime)).".</td>
            </tr>
            <tr><td colspan='2'>&nbsp;</td></tr>
            <tr>
            ...
            $smsbody = "Your employer has approved request for ".$row['extra_hours']." extra hours for the job: ".stripslashes($fetch['title']).". New end time is: ".date('M j, Y', strtotime($newenddate))." at ".date('h:i a', strtotime($newendtime)).".";
