							  ...
							  <div class="tab-pane fade <?php echo $tab1_content_active;?>" id="pills-home" role="tabpanel" aria-labelledby="pills-home-tab">
								...
								<div class="table-responsive successtable">
									...
									<table class="table" cellpadding="5" cellspacing="5">
										...
										$mytotal_cover = $jobs = dbQuery($dbConn, "SELECT a.id from job_details a inner join job_status b on a.id=b.jobid where b.contacting=1 and b.application_sent_to = '".$_SESSION['loginUserId']."' order by b.id desc");
										$total_cover = dbNumRows($mytotal_cover);
										
										$jobs = dbQuery($dbConn, "SELECT a.id,a.title,a.street_address,a.postdate,a.payperhr,a.job_status,a.jobdate,a.jobdate2,a.starttime,a.endtime,a.isclosed,a.paytype,a.payperhr_max,a.annual,a.commission_perctg,a.annualamt,a.commission_perctg_amt,a.add_time,a.covertype,a.longstartdt,b.hired from job_details a inner join job_status b on a.id=b.jobid where b.contacting=1 and b.application_sent_to = '".$_SESSION['loginUserId']."' order by b.id desc limit ".$start.",".$records_per_page."");
										if(dbNumRows($jobs) > 0){
										while($row = dbFetchArray($jobs)){
											...
											if($row['covertype']==1){
											?>
											<br>Start Date: <?php echo date('F j, Y', strtotime($row['jobdate']));?><br>End Date: <?php 
											if($row['jobdate2'] != "0000-00-00")
												echo date('F j, Y', strtotime($row['jobdate2']));
											else
												echo date('F j, Y', strtotime($row['jobdate']))
											?>
											<?php }
											...
											?>
											<a href="javascript:void(0);" class="editavl desktopvw" id="avl_<?php echo $row['id'];?>_<?php echo $_SESSION['loginUserId'];?>"><i class="fa fa-pencil" aria-hidden="true"></i> &nbsp;Edit Availability</a>
											<a href="javascript:void(0);" class="editavl mobilvw" id="avl_<?php echo $row['id'];?>_<?php echo $_SESSION['loginUserId'];?>"><i class="fa fa-pencil" aria-hidden="true"></i></a>
											<?php
											}
											?>
											</td>
											...
											echo "<span class='txtclos'><i class='fa fa-circle' aria-hidden='true'></i> &nbsp;Paused</span>";
											?>
											</span></td>
											</tr>
											<?php
										...
										for($i = 1; $i<=ceil($total_cover/$records_per_page); $i++){
											...
											<?php
												...
												?>
													<li><a href="<?php echo SITEURL;?>myappliedjobs/?page=<?php echo $i;?>"><?php echo $i;?></a></li>
												<?php
											}
										}
										?>
							...
							?>
										...
										<?php
											...
											<div class="table-responsive">
												<table class="table">
													<tr>
														<td>You are not confirmed for any job yet.</td>
													</tr>
											</table>
											</div>
											</div>
